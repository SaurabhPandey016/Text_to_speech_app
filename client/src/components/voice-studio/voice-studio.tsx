...
export function VoiceStudio() {
  ...
  const [voice, setVoice] = useState("Riya");
  ...
  async function submitAuth(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${API_URL}/api/auth/${authMode === "login" ? "login" : "register"}`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
    ...
    <main className="site-shell">
      ...
      <section className="studio-grid" aria-label="Text to speech studio">
        ...
        <div className="settings-panel panel reveal-up delay-two">
          ...
          <div className="select-wrap"><select id="language" value={language} onChange={(event) => { const nextLanguage = event.target.value; setLanguage(nextLanguage); const nextVoice = voices.find((item) => item.code === nextLanguage)?.id || ""; setVoice(nextVoice); }}>{availableLanguages.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select><ChevronDown size={17} /></div>
      ...
      {authOpen && (
        <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setAuthOpen(false); }}>
          <div className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title">
            ...
            {user && authMode === "account" ? (
              <>
                <span className="section-kicker">Your account</span>
                <h2 id="auth-title">Welcome back, {user.name.split(" ")[0]}.</h2>
                <p>Your listening room is ready and your recent audio stays saved.</p>
                <div className="account-panel">
                  <div className="account-summary"><span className="account-pill"><CircleUserRound size={16} /></span><div><strong>{user.name}</strong><small>{user.email}</small></div></div>
                  <button className="account-action">Manage profile</button>
                  <button className="account-action">Recent generations</button>
                  <button className="generate-button account-logout" onClick={logout}>Sign out</button>
                </div>
              </>
            ) : (
              <>
                <span className="section-kicker">Your listening room</span>
                <h2 id="auth-title">{authMode === "login" ? "Welcome back." : "Make Echo yours."}</h2>
                <p>{authMode === "login" ? "Sign in to keep your recent audio close." : "Create an account to keep a private generation history."}</p>
                <form onSubmit={submitAuth}>
                  {authMode === "register" && <input required minLength={2} placeholder="Your name" value={authForm.name} onChange={(event) => setAuthForm({ ...authForm, name: event.target.value })} />}
                  ...
                  <input required minLength={8} type="password" placeholder="Password · 8 characters minimum" value={authForm.password} onChange={(event) => setAuthForm({ ...authForm, password: event.target.value })} />
                  {authError && <div className="error-message">{authError}</div>}
                  <button className="generate-button" disabled={authBusy}>{authBusy ? <LoaderCircle size={19} className="spin" /> : <Zap size={18} />}{authBusy ? "Please wait..." : authMode === "login" ? "Sign in" : "Create account"}</button>
                </form>
                <button className="switch-auth" onClick={() => setAuthMode(authMode === "login" ? "register" : "login")}>{authMode === "login" ? "Need an account? Create one" : "Already have an account? Sign in"}</button>
              </>
            )}
          </div>
        </div>
